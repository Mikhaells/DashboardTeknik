...
interface DetailLaporanTeknikalDirektorFormProps {
  laporanId?: number;
  onClose: () => void;
  onSuccess: () => void;
}

export default function DetailLaporanTeknikalDirektorForm({ laporanId, onClose, onSuccess }: DetailLaporanTeknikalDirektorFormProps) {
  ...
  const fetchUploadedImages = async (id: number) => {
    try {
      ...
      const response = await fetch(`/api/laporan-teknikal-direktor/${id}/images`);
  ...
  const generatePrintHtml = async () => {
    ...
    return `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Detail Laporan Teknikal Direktor - ${formData.TanggalProduksi || 'Laporan'}</title>
        <style>
          * {
            margin: 0;
            padding: 0;
      ...
      <body>
        <div class="print-container">
          <div class="header">
            <h2>DETAIL LAPORAN TEKNIKAL DIREKTOR</h2>
            <p>LAPORAN PRODUKSI TEKNIKAL DIREKTOR</p>
          </div>

          <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">A. INFORMASI PRODUKSI</h3>
            </div>
            <table class="label-table">
              <tr>
                <td>Nama Program</td>
                <td>${formData.NamaProgram || '-'}</td>
              </tr>
              <tr>
                <td>Jenis Produksi</td>
                <td>${formData.JenisProduksi || '-'}</td>
              </tr>
              <tr>
                <td>Tanggal Produksi</td>
                <td>${formData.TanggalProduksi || '-'}</td>
              </tr>
              <tr>
              ...
              <tr>
                <td>Nama Technical Director</td>
                <td>${formData.NamaTechnicalDirector || '-'}</td>
              </tr>
              <tr>
                <td>Nama PDU</td>
                <td>${formData.NamaPDU || '-'}</td>
              </tr>
            </table>
          </div>
          ...
          <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">B. SPESIFIKASI TEKNIS</h3>
            </div>
            <table class="label-table">
              <tr>
                <td>Jumlah Kamera</td>
                <td>${formData.JumlahKamera || '0'}</td>
              </tr>
              <tr>
                <td>Format Video</td>
                <td>${formData.FormatVideo || '-'}</td>
              </tr>
              <tr>
                <td>Resolusi Output</td>
                <td>${formData.ResolusiOutput || '-'}</td>
              </tr>
              <tr>
                <td>Audio Output</td>
                <td>${formData.AudioOutput || '-'}</td>
              </tr>
              <tr>
                <td>Media Recording</td>
                <td>${formData.MediaRecording || '-'}</td>
              </tr>
              <tr>
                <td>Jalur Distribusi</td>
                <td>${formData.JalurDistribusi || '-'}</td>
              </tr>
            </table>
          </div>
           ...
           <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">C. ALAT PRODUKSI</h3>
            </div>
            <table class="label-table" style="margin-top: 10px;">
              <thead>
                <tr style="background-color: #f5f5f5;">
                  ...
                  <th style="border: 1px solid #000; padding: 8px 10px; font-size: 10pt; font-weight: 600; text-align: left;">Alat Produksi</th>
              ...
              <tbody>
                ${detailItems.map((item, index) => {
                  const alatNama = ALAT_PRODUKSI_LIST.find(a => a.id === item.AlatProduksi)?.nama || `Alat #${item.AlatProduksi}`;
                  return `
                  <tr>
                    <td style="border: 1px solid #000; padding: 8px 10px; font-size: 10pt; text-align: center;">${index + 1}</td>
                    <td style="border: 1px solid #000; padding: 8px 10px; font-size: 10pt;">${alatNama}</td>
                    ...
                    <td style="border: 1px solid #000; padding: 8px 10px; font-size: 10pt;">${item.Keterangan || '-'}</td>
                  </tr>
                  `;
                }).join('')}
              </tbody>
          ...
          <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">D. KENDALA & TINDAKAN</h3>
            </div>
            <div class="print-textarea">${(formData.KendalaTindakan || '-').replace(/\n/g, '<br>')}</div>
          </div>

          ${uploadedImages.length > 0 ? `
          <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">E. ATTACHMENT</h3>
            </div>
          ...
          <div class="section-wrapper">
            <div class="section-title-wrapper">
              <h3 class="section-title">F. FEEDBACK</h3>
            </div>
        ...
        <form>
              ...
              <tbody>
                <tr className="border border-gray-300">
                  ...
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="NamaProgram" value={formData.NamaProgram} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Jenis Produksi</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="JenisProduksi" value={formData.JenisProduksi} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Tanggal Produksi</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="date" name="TanggalProduksi" value={formData.TanggalProduksi} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Lokasi Produksi</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="LokasiProduksi" value={formData.LokasiProduksi} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Nama Technical Director</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="NamaTechnicalDirector" value={formData.NamaTechnicalDirector} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Nama PDU</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="NamaPDU" value={formData.NamaPDU} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <table className="w-full border-collapse">
              <tbody>
                <tr className="border border-gray-300">
                  ...
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="number" name="JumlahKamera" value={formData.JumlahKamera} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Format Video</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="FormatVideo" value={formData.FormatVideo} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Resolusi Output</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="ResolusiOutput" value={formData.ResolusiOutput} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Audio Output</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="AudioOutput" value={formData.AudioOutput} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Media Recording</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="MediaRecording" value={formData.MediaRecording} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
                <tr className="border border-gray-300">
                  <td className="font-semibold w-1/3 bg-blue-50 text-blue-900 px-3 py-2 border border-gray-300">Jalur Distribusi</td>
                  <td className="px-3 py-2 border border-gray-300 bg-white text-gray-900">
                    <input type="text" name="JalurDistribusi" value={formData.JalurDistribusi} onChange={handleInputChange} className="w-full px-2 py-1 text-sm border-none outline-none focus:outline-2 focus:outline-blue-500 placeholder-gray-600 text-gray-900 bg-gray-50" readOnly />
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="mb-8">
                  ...
                  {detailItems.map((item, index) => (
                    <tr key={index} className="border border-gray-300 hover:bg-gray-50">
                      ...
                      <td className="border border-gray-300 px-3 py-2 text-center">
                        <input type="checkbox" checked={item.Check} disabled className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
                      </td>
                      <td className="border border-gray-300 px-3 py-2 text-gray-700">{item.Keterangan || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Kendala dan tindakan yang dilakukan"
          ...
          {uploadedImages.length > 0 && (
            <div className="mb-8">
              <h3 className="text-sm font-bold bg-green-50 text-green-900 px-4 py-2 rounded mb-4">E. ATTACHMENT</h3>
              <div className="ml-6">
          ...
          {(formData.StatusLaporanId === 5 || formData.StatusLaporanId === 6) && (
            <div className="mb-8">
            <h3 className="text-sm font-bold bg-red-50 text-red-900 px-4 py-2 rounded mb-4">F. FEEDBACK</h3>
            <textarea
              name="Feedback"
