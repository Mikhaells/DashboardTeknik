...
import { getStatusName, getStatusColor, isPending, isAdminEditable, isManagerEditable } from '@/lib/status';
import Pagination from '../Pagination';
import DetailLaporanTeknikalDirektorForm from './DetailLaporanTeknikalDirektorForm';
import FeedbackModal from '../FeedbackModal';
import TambahLaporanTeknikalDirektorForm from './TambahLaporanTeknikalDirektorForm';
import EditLaporanTeknikalDirektorForm from './EditLaporanTeknikalDirektorForm';

interface LaporanTeknikalDirektorDataTableProps {
...
export default function LaporanTeknikalDirektorDataTable({ data, pagination, onPageChange, userLevelId = 2, searchTerm = '', onSearchChange, onSearch, activeFilter = 'all' }: LaporanTeknikalDirektorDataTableProps) {
  ...
  const handleApproveSubmit = async (feedback: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/laporan-teknikal-direktor/${selectedLaporanId}/approve`, {
        method: 'POST',
      ...
      if (result.success) {
        alert('Laporan teknikal direktor berhasil di-approve!');
        setShowApproveModal(false);
        setSelectedLaporanId(null);
  ...
  const handleRejectSubmit = async (feedback: string) => {
    ...
    try {
      setIsProcessingFeedback(true);
      const response = await fetch(`/api/laporan-teknikal-direktor/${selectedLaporanId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Catatan: feedback }),
      });
      const result = await response.json();
      if (result.success) {
        alert('Laporan teknikal direktor berhasil di-reject!');
        setShowRejectModal(false);
        setSelectedLaporanId(null);
        window.location.reload();
      } else {
  ...
  const handlePrintByDate = async () => {
    ...
    try {
      const response = await fetch(`/api/laporan-teknikal-direktor/print-all-approved?startDate=${startDate}&endDate=${endDate}`);
      const result = await response.json();
      ...
      const dataWithImages = await Promise.all(
        result.data.map(async (item: any) => {
          try {
            ...
            const imgResponse = await fetch(`/api/laporan-teknikal-direktor/${laporanId}/images`);
  ...
  const generateSingleReportHtml = (item: any) => {
    ...
    const detailItems = item.LaporanTeknikalDirektor_Detail || [];
    ...
    return `
      ...
      <head>
        <meta charset="UTF-8">
        <title>LAPORAN TEKNIKAL DIREKTOR - ${l.TanggalProduksi ? formatDateForPrint(l.TanggalProduksi) : 'Laporan'}</title>
        <style>
      ...
      <body>
        ...
        <div class="section-wrapper">
          ...
          <table class="label-table">
            <tr><td>Nama Program</td><td>${l.NamaProgram || '-'}</td></tr>
            <tr><td>Jenis Produksi</td><td>${l.JenisProduksi || '-'}</td></tr>
            <tr><td>Tanggal Produksi</td><td>${l.TanggalProduksi ? formatDateForPrint(l.TanggalProduksi) : '-'}</td></tr>
            <tr><td>Lokasi Produksi</td><td>${l.LokasiProduksi || '-'}</td></tr>
            <tr><td>Nama Technical Director</td><td>${l.NamaTechnicalDirector || '-'}</td></tr>
            <tr><td>Nama PDU</td><td>${l.NamaPDU || '-'}</td></tr>
          </table>
        </div>
        ...
        <div class="section-wrapper">
          ...
          <table class="label-table">
            <tr><td>Jumlah Kamera</td><td>${l.JumlahKamera || '0'}</td></tr>
            <tr><td>Format Video</td><td>${l.FormatVideo || '-'}</td></tr>
            <tr><td>Resolusi Output</td><td>${l.ResolusiOutput || '-'}</td></tr>
            <tr><td>Audio Output</td><td>${l.AudioOutput || '-'}</td></tr>
            <tr><td>Media Recording</td><td>${l.MediaRecording || '-'}</td></tr>
            <tr><td>Jalur Distribusi</td><td>${l.JalurDistribusi || '-'}</td></tr>
          </table>
        </div>
          ...
          <table class="label-table" style="margin-top: 10px;">
            ...
            <tbody>
              ${detailItems.map((d: any, i: number) => {
                const alatNama = ALAT_PRODUKSI_LIST.find(a => a.id === d.AlatProduksi)?.nama || `Alat #${d.AlatProduksi}`;
                return `
                <tr>
                  ...
                  <td style="border: 1px solid #000; padding: 8px 10px; font-size: 10pt;">${d.Keterangan || '-'}</td>
                </tr>
                `;
              }).join('')}
            </tbody>
        ...
        <div class="section-wrapper">
          ...
          <div class="print-textarea">${(l.KendalaTindakan || '-').replace(/\n/g, '<br>')}</div>
        </div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="px-4 py-3 sm:px-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              Data Laporan Teknikal Direktor
            </h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">TD</th>
              ...
              {data.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="text-gray-500">
                      <p>Belum ada data laporan teknikal direktor.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                data.map((laporan) => (
                  <tr key={laporan.Id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{laporan.NamaProgram}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(laporan.TanggalProduksi)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{laporan.NamaTechnicalDirector}</td>
      ...
      {showModal && (
        <TambahLaporanTeknikalDirektorForm
      ...
      {showDetailModal && selectedLaporanId && (
        <DetailLaporanTeknikalDirektorForm
      ...
      {showEditModal && selectedLaporanId && (
        <EditLaporanTeknikalDirektorForm
          laporanId={selectedLaporanId}
      ...
      <FeedbackModal
        ...
        message="Apakah Anda yakin ingin menyetujui laporan teknikal direktor ini? Berikan feedback untuk pembuat laporan."
      ...
      <FeedbackModal
        ...
        title="Tolak Laporan"
        message="Apakah Anda yakin ingin menolak laporan teknikal direktor ini? Berikan feedback untuk pembuat laporan."
