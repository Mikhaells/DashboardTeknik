...
function getUploadDir(laporanId: string): string {
  return join(UPLOAD_BASE_PATH, 'laporan-teknikal-direktor', laporanId);
}

function getPublicUploadDir(laporanId: string): string {
  return join(PUBLIC_UPLOAD_PATH, 'laporan-teknikal-direktor', laporanId);
}
...
export async function GET(
  ...
  try {
    ...
    const imageFiles = files
      ...
      .sort()
      .map(file => {
        const relativeUrl = `${urlPrefix}/laporan-teknikal-direktor/${laporanId}/${file}`;
