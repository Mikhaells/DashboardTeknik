...
import { getLaporanByDateRange } from '@/lib/laporan-maintenance';
  ...
  try {
    ...
    const detailList = await executeQuery<any>(
      `SELECT LaporanMaintenanceId, Kegiatan, [Check], [Desc]
       ...
       WHERE LaporanMaintenanceId IN (${idParams})`,
      idParamObj
    );

    const detailByLaporanId: Record<number, any[]> = {};
    (detailList || []).forEach((item: any) => {
      if (!detailByLaporanId[item.LaporanMaintenanceId]) detailByLaporanId[item.LaporanMaintenanceId] = [];
      detailByLaporanId[item.LaporanMaintenanceId].push(item);
    });

    const data = laporanList.map((laporan) => ({
      laporan,
      LaporanMaintenance_Detail: (detailByLaporanId[laporan.Id] || []).map((item: any) => ({
    ...
    console.error('Error fetching approved laporan maintenance for print:', error);
    return NextResponse.json(
      {
        success: false,
