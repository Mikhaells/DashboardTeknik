...
/**
 * API Route: POST /api/laporan-genset/[id]/approve
 * 
 * Handles approving laporan genset by ID
 */
export async function POST(
  ...
  try {
    ...
    // Only allow admin (LevelId: 1) to approve
    ...
    // Only allow approve if status is in PENDING_STATUSES (2, 3, 4)
    ...
    // Update laporan status to approved (5) with feedback
    const updateQuery = `
    ...
    return NextResponse.json({
      success: true,
      message: 'Laporan genset berhasil disetujui',
    });

  } catch (error) {
    console.error('Error approving laporan genset:', error);
    return NextResponse.json(
      {
        success: false,
